...
export default function Header() {
  ...
  const navItems = ["New In", "Bestsellers", "Women's Apparel", "Accessories", "Jewelry"];
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-line bg-white">
      ...
      <nav className="hidden md:flex mx-auto max-w-6xl items-center gap-7 px-6 pb-3.5 text-[13.5px] font-semibold">
        ...
        {navItems.map((i) => (
          <a key={i} href="#" className={NAV_LINK}>{i}</a>
        ))}
      ...
      {mobileNavOpen && (
        <div className="md:hidden flex flex-col gap-3 border-t border-line px-6 pb-5">
          {["Shop", ...navItems, "Beige Botany", "Top Fall Picks", "Vendors", "The Edit"].map((i) => (
            <a key={i} href="#" className="pt-2.5 text-sm font-semibold">{i}</a>
          ))}
